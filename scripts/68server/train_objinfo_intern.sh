...
# ===== 数据集路径（实验 5: object-info prompt） =====
SPLIT_ROOT="${WORKDIR}/dataset_scene_v7_full50_rotation8_trainready_front2others_splitobj_seed42_bboxmask_bright_objinfo_20260418"
...
OUTPUT_DIR_LIST=(
  "${WORKDIR}/DiffSynth-Studio/output/rotation8_bright_objinfo_rank32"
)
...
EVAL_OBJECT_DESCRIPTION="wooden chair"
...
for i in "${!CSV_LIST[@]}"; do
  ...
  echo ""
  echo "=================================================================="
  echo "开始训练任务 #${idx} (intern 8xH100, object-info prompt)"
  echo "METADATA: ${csv}"
  echo "OUT: ${out}"
  echo "TB : ${tb_dir}"
  echo "EVAL IMAGE: ${EVAL_IMAGE_PATH}"
  echo "EVAL OBJECT: ${EVAL_OBJECT_DESCRIPTION}"
  echo "=================================================================="
