#!/bin/bash
# SpatialEdit-Bench VIEScore evaluation — 4 modes parallel on 4 GPUs
set -e
...
# Phase 2: Launch 4 parallel VIEScore evaluations
echo ""
echo "Phase 2: VIEScore evaluation (4 GPUs parallel)"
...
echo ""
echo "Waiting for all 4 evaluations..."

for i in 0 1 2 3; do
    wait ${PIDS[$i]}
    echo "  ${MODES[$i]} done (exit $?)"
done
