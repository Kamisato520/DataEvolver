...
Bridge Codex-first DataEvolver workflows to the local Claude Code CLI.
...
- By default the reviewer gets **no tools**. This matches the original DataEvolver pattern where the external reviewer only sees the prompt context prepared by the executor.
...
- Use `review_start` / `review_reply_start` + `review_status` for long dataset/evaluator project reviews. This avoids the observed `Codex -> tools/call` timeout around 120 seconds.
...
```json
{
  "name": "review_start",
  "arguments": {
    "prompt": "Review this dataset synthesis + evaluator VLM plan..."
  }
}
```
