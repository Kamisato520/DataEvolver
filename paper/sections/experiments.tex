...
We construct two versions of the 50-object DataEvolver-Rotate rendering set:
...
\begin{itemize}
  ...
  \item \textbf{Full pipeline (VLM evolution):} The complete \dataevolver{} loop is applied,
...
\begin{figure}[t]
  ...
  \caption{
    ...
    and the full \dataevolver{} evolution pipeline.
...
We evaluate the downstream training value of DataEvolver-Rotate by fine-tuning
...
\begin{enumerate}
  ...
  \item \textbf{LoRA (full pipeline data)}: LoRA fine-tuned on quality-approved data
    from the full \dataevolver{} evolution pipeline.
\end{enumerate}
...
\begin{table}[h]
\caption{
  Quantitative evaluation of rotation editing quality on the DataEvolver-Rotate test set.
...
\begin{figure}[t]
  ...
  \caption{
    ...
    LoRA fine-tuned on DataEvolver-Rotate data produces more geometrically consistent
